...
const formatNumber = (raw, brand) => {
  ...
  let i = 0; const out = [];
  for (const g of groups) { if (i>=v.length) break; out.push(v.slice(i,i+g)); i+=g; }
  return out.join(" ");
};
      ...
      <div className="relative [perspective:1000px]">
        <div className={`relative h-44 w-full rounded-xl shadow-lg transition-transform duration-500 [transform-style:preserve-3d] ${showBack ? "rotate-y-180":""}`}>
          ...
          <div className="absolute inset-0 rounded-xl bg-zinc-900 text-white p-5 [backface-visibility:hidden]">
            <div className="text-xs opacity-60">{brand?.name || "Card"}</div>
            <div className="mt-6 text-lg tracking-widest font-medium">{printed || "•••• •••• •••• ••••"}</div>
            <div className="mt-6 flex items-center justify-between text-xs">
              <div><div className="opacity-60">Cardholder</div><div className="font-medium">{name || "—"}</div></div>
              <div><div className="opacity-60">Exp.Date</div><div className="font-medium">{expiry || "MM/YY"}</div></div>
            </div>
          ...
          <div className="absolute inset-0 rounded-xl bg-zinc-800 text-white p-5 rotate-y-180 [backface-visibility:hidden]">
            ...
            <div className="mt-6 flex items-center gap-3">
              <div className="flex-1"><div className="h-8 w-full rounded bg-white/90" /></div>
...
export default function PaymentStep({ items, address, shipping, money, card, setCard }) {
  /* ===== Summary (igual que antes) ===== */
  const pm = useMemo(() => { const m=new Map(); tiles.forEach(t=>m.set(t.id,t)); return m; }, []);
        ...
        <aside className="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm">
            ...
            {items.map(i=>{
              ...
              return (
                  <li key={`${i.id}-${i.variant??""}`} className="flex items-center gap-3 rounded-lg bg-zinc-50 p-2">
                    {img ? <img src={img} alt={title} className="h-10 w-10 rounded-md object-cover border border-zinc-200" /> : <div className="h-10 w-10 rounded-md bg-zinc-200" />}
                    <div className="flex-1 min-w-0">
                      <p className="truncate text-sm font-medium text-zinc-900">{title}</p>
                      <p className="text-xs text-zinc-500">Qty {i.quantity??1}</p>
                    </div>
                    <span className="rounded-full bg-white px-3 py-1 text-xs font-semibold text-zinc-900 border border-zinc-200">{money(price)}</span>
                  </li>
              );
            })}
          </ul>
          ...
          <div className="mt-4 space-y-1 text-sm">
            ...
            <p className="font-medium text-zinc-900">
              {address==="home" ? "2118 Thornridge Cir, Syracuse, CT 35624"
                  : address==="office" ? "2715 Ash Dr. San Jose, SD 83475" : "—"}
            </p>
          </div>

          <div className="mt-2 space-y-1 text-sm">
            <p className="text-zinc-500">Shipment method</p>
            <p className="font-medium text-zinc-900 capitalize">{shipping || "—"}</p>
          </div>

          <div className="mt-4 border-t pt-4 text-sm space-y-2">
            <div className="flex justify-between"><span>Subtotal</span><span>{money(subtotal)}</span></div>
            <div className="flex justify-between"><span>Estimated Tax</span><span>{money(tax)}</span></div>
            <div className="flex justify-between"><span>Estimated shipping &amp; Handling</span><span>{money(shippingCost)}</span></div>
        ...
        <div className="space-y-4">
          <div className="flex gap-2">
            {["card","paypal","ppc"].map(id=>(
                <button key={id} type="button" onClick={()=>setMethod(id)}
                  ...
                  {id==="card"?"Credit Card":id==="paypal"?"PayPal":"PayPal Credit"}
                </button>
            ))}
          </div>
          ...
          {method==="card" ? (
              <div className="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm space-y-5">
                ...
                <div className="pt-2 space-y-3">
                  ...
                  <div className={`transition-opacity ${step>0?"opacity-50":"opacity-100"}`}>
                    <label className="mb-1 block text-xs text-zinc-600">Card Number {brand ? `· ${brand.name}` : ""}</label>
                    <input
                        id="cc-number" type="text" inputMode="numeric" placeholder="•••• •••• •••• ••••"
                    ...
                    <div className="mt-1 text-[11px] text-zinc-500">
                      {brand ? `${brand.total} dígitos • CVV ${cvvLen}` : "Visa / Mastercard / AmEx"}
                    </div>
                  </div>

                  {/* Nombre */}
                  <div className={`transition-opacity ${step>1?"opacity-50": step===1?"opacity-100":"opacity-50"}`}>
                    <label className="mb-1 block text-xs text-zinc-600">Cardholder Name</label>
                    <input
                        id="cc-name" type="text" placeholder="NOMBRE APELLIDO"
                        value={card.name||""}
                  ...
                  <div className={`transition-opacity ${step===2?"opacity-100":"opacity-50"}`}>
                    <label className="mb-1 block text-xs text-zinc-600">Exp. Date</label>
                    <input
                        id="cc-expiry" type="text" inputMode="numeric" placeholder="MM/YY"
                        value={card.expiry||""}
                  ...
                  {/* CVV (flip automático al llegar a este paso) */}
                  <div className={`transition-opacity ${step===3?"opacity-100":"opacity-50"}`}>
                    ...
                    <input
                        id="cc-cvv" type="text" inputMode="numeric" placeholder={brand?.id==="amex"?"1234":"123"}
                  ...
                  <div className="flex flex-wrap gap-2 pt-1">
                    <button type="button" onClick={()=>fill("visa")}   className="rounded-md border border-zinc-300 bg-white px-3 py-1.5 text-xs">Autocompletar VISA</button>
                    <button type="button" onClick={()=>fill("master")} className="rounded-md border border-zinc-300 bg-white px-3 py-1.5 text-xs">Autocompletar MASTERCARD</button>
                    <button type="button" onClick={()=>fill("amex")}   className="rounded-md border border-zinc-300 bg-white px-3 py-1.5 text-xs">Autocompletar AMEX</button>
                  </div>

                  {/* Estado de validación (sin botones de avance) */}
              ...
              <div className="rounded-xl border border-zinc-200 bg-white p-6 text-sm text-zinc-600">
                Elegiste {method==="paypal"?"PayPal":"PayPal Credit"}. En producción te redirigiría al proveedor.
              </div>
          )}
